...
static int route4_stats_show(struct gr_api_client *c, uint16_t vrf_id, struct gr_table *table) {
	struct gr_ip4_route_stats_list_req req = {.vrf_id = vrf_id};
	const struct gr_ip4_route_stats *stats;
	int ret;

	gr_api_client_stream_foreach (stats, ret, c, GR_IP4_ROUTE_STATS_LIST, sizeof(req), &req) {
