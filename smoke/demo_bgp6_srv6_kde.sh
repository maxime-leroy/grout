...
DEMO_SIGNAL="demo-bgp6-srv6"
STEP=0
TOTAL=10
cleanup() {
	rm -f "$TOPO_FILE" /tmp/next-demo.sh /tmp/quit-demo.sh
	tmux unbind -n F12
	tmux unbind -n F5
...
# Create helper scripts to control the demo from inside the container.
# Example from the host:
#   docker exec grout-frr-u24 /tmp/next-demo.sh
#   docker exec grout-frr-u24 /tmp/quit-demo.sh
#
# They use the exact tmux socket of the running demo session.
TMUX_SOCKET=${TMUX%%,*}
[ -n "$TMUX_SOCKET" ] || {
	echo "TMUX socket not found; this demo must run inside tmux" >&2
	exit 1
}

cat > /tmp/next-demo.sh <<EOF
#!/bin/sh
exec tmux -S "$TMUX_SOCKET" set -g @demo_action next \; wait-for -S "$DEMO_SIGNAL"
EOF
chmod +x /tmp/next-demo.sh

cat > /tmp/quit-demo.sh <<EOF
#!/bin/sh
exec tmux -S "$TMUX_SOCKET" set -g @demo_action quit \; wait-for -S "$DEMO_SIGNAL"
EOF
chmod +x /tmp/quit-demo.sh
...
run "$PANE_GROUT" "grcli interface show name main"
CLEAR=false
